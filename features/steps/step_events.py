...
@when(
    'a search for "{keywords}" keywords requesting availability with performances is performed'
)
@vcr.use_cassette("fixtures/cassettes/search-keyword.yaml", record_mode="new_episodes")
...
@when(
    'a search for events with performances "{start_days}"-"{end_days}" days from now is performed'
)
@vcr.use_cassette(
    "fixtures/cassettes/search-daterange.yaml", record_mode="new_episodes"
)
...
@when('a search for events in country with code "{country_code}" is performed')
...
@when(
    'a search for events within "{radius}"km of "{latitude}" lat and "{longitude}" long is performed'
)
...
@when('we attempt to fetch events with the ID\'s "{event_ids}"')
@vcr.use_cassette(
    "fixtures/cassettes/get-events-single.yaml", record_mode="new_episodes"
)
...
@when('we attempt to fetch events with the ID\'s "{event_ids}" requesting availability')
@vcr.use_cassette(
    "fixtures/cassettes/get-events-single.yaml", record_mode="new_episodes"
)
...
@when(
    'we attempt to fetch events with the ID\'s "{event_ids}" requesting availability with performances'
)
@vcr.use_cassette(
    "fixtures/cassettes/get-events-single.yaml", record_mode="new_episodes"
)
...
@when('we attempt to fetch events with the ID\'s "{event_ids}" requesting extra info')
@vcr.use_cassette(
    "fixtures/cassettes/get-events-single.yaml", record_mode="new_episodes"
)
...
@when('we attempt to fetch events with the ID\'s "{event_ids}" requesting reviews')
@vcr.use_cassette(
    "fixtures/cassettes/get-events-single.yaml", record_mode="new_episodes"
)
...
@when('we attempt to fetch events with the ID\'s "{event_ids}" requesting media')
@vcr.use_cassette(
    "fixtures/cassettes/get-events-single.yaml", record_mode="new_episodes"
)
...
@when('we attempt to fetch events with the ID\'s "{event_ids}" with add-ons')
@vcr.use_cassette(
    "fixtures/cassettes/get-events-single.yaml", record_mode="new_episodes"
)
...
@when('we attempt to fetch events with the ID\'s "{event_ids}" with upsells')
@vcr.use_cassette(
    "fixtures/cassettes/get-events-single.yaml", record_mode="new_episodes"
)
...
@vcr.use_cassette(
    "fixtures/cassettes/event-performances-date-range.yaml", record_mode="new_episodes"
)
def then_events_have_performance_between_days(context, start, end):
    ...
    for event in context.events:
        ...
        if meta.auto_select:
            continue
        if any(
            performance.date_time >= start_date and performance.date_time <= end_date
            for performance in performances
        ):
            continue

        raise Exception(
            "Event with id %s does not have a performance inside the given date range"
            % event.id
        )
...
def then_events_are_all_within_distance_of_lat_long(context, distance, lat, lon):
    ...
    for event in context.events:
        ...
        a = (
            ...
            + math.cos(rad_lat_origin)
            * math.cos(rad_lat_event)
        ...
        raise Exception(
            "Event with id %s and coordinates %f, %f is more than %d km from the given position"
            % (event.id, event.latitude, event.longitude, distance)
        )


@then('all events are in country with code "{country_code}"')
@vcr.use_cassette("fixtures/cassettes/search-country.yaml", record_mode="new_episodes")
...
@then('those events should have the ID\'s "{event_ids}"')
...
@then("the event has content information")
...
@then("the event has event information")
...
@then("the event has event information starting with")
def then_the_event_has_event_info_starting_with(context):
    ...
    assert_that(
        context.event.event_info_html[:sample_size], equal_to(expected["value_html"])
    )


@then("the event has venue information")
...
@then('the event has "{reviews}" reviews')
...
@then('the upsells contain "{event_ids}"')
...
@then('the upsells do not contain "{event_ids}"')
...
@then("the event needs a performance to be selected")
def then_the_event_needs_a_performance_to_be_selected(context):
    assert context.event.needs_performance


@then("the event does not need a performance to be selected")
