...
def then_the_purchase_response_is_the_same_as_the_original(context):
    ...
    assert (
        context.status.trolley.transaction_uuid
        == context.get_purchase.trolley.transaction_uuid
    )
    assert (
        context.status.trolley.order_count == context.get_purchase.trolley.order_count
    )
