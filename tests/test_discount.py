...
class TestDiscount:
    ...
    def test_from_api_data(self):
        ...
        assert (
            discount.gross_commission.including_vat
            == gross_commission_data["amount_including_vat"]
        )
        assert (
            discount.gross_commission.excluding_vat
            == gross_commission_data["amount_excluding_vat"]
        )
        assert (
            discount.gross_commission.currency_code
            == gross_commission_data["commission_currency_code"]
        )
        assert isinstance(discount.user_commission, Commission)
        assert (
            discount.user_commission.including_vat
            == user_commission_data["amount_including_vat"]
        )
        assert (
            discount.user_commission.excluding_vat
            == user_commission_data["amount_excluding_vat"]
        )
        assert (
            discount.user_commission.currency_code
            == user_commission_data["commission_currency_code"]
        )
